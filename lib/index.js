...
const plsqlLinter = function() {
  ...
  const supportedServerVersion = '1.0.2'
  ...
  function setConfigFiles() {
    ...
    function watchConfig(projectDir) {
      ...
      pDir // clean up orphened files
        ...
        .forEach((config) => {
          const key = config.getPath()
          disposableConfigs[key].dispose()
          delete configs[key]
        })
    }
    ...
    atom.project.getDirectories().forEach((dir) => {
      ...
      disposables.push(dir.onDidChange(() => watchConfig(dir)))
    })
  }
  ...
  function getFilters(path) {
    ...
    if (configs[projectKey] != null && configs[projectKey].filters != null)
