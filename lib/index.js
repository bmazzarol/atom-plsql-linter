    ...
    return {
        ...
        activate() {
            ...
            isRunning(null, () => {
                ...
                exec((isWin ? command.replace('/', '\\') + '.bat' : command) + ' ' + atom.config.get('plsql-linter.port'), (error, stdout, stderr) => {
                    if (error) {
            ...
            return {
                ...
                lint(editor) {
                    ...
                    return new Promise(function(resolve, error) {
                        isRunning(() => callServer('/lint-file', 'POST', fileText, (resp) => {
                            var errors = ''
                            ...
                            resp.on('end', () => {
                                ...
                                if (errors != null && errors != '') {
                                    ea = JSON.parse(errors).map((m) => {
                                        m.location.file = filePath
                                        return m
                                    })
                                }
                                resolve(ea)
